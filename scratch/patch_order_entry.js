...
export function OrderEntry({ symbol }: { symbol: string }) {
  ...
  useEffect(() => {
    if (user?.portfolioId) {
      apiFetch('/portfolios/' + user.portfolioId, { method: 'GET' })
        .then(res => res.json())
        .then(data => setIsMarginEnabled(data.isMarginEnabled === true))
        .catch(console.error);
    }
  }, [user]);
      ...
      <form onSubmit={submitOrder} className="space-y-4">
        <div className="flex space-x-2">
          ...
          <Button 
            ...
            aria-label={isMarginEnabled ? "Sell / Short" : "Sell"}
          >
            {isMarginEnabled ? "Sell / Short" : "Sell"}
          </Button>
        </div>
        ...
        {message && (
          <p className="text-sm mt-2 text-center text-muted-foreground" aria-live="polite">{message}</p>
        )}
      </form>
    </div>
  );
